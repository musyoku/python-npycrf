...
namespace npycrf {
	namespace crf {
		...
		int CRF::_hash_unigram_b(int y_i_1, int y_i, int i, int x_i){
			assert(x_i < _num_character_ids);
			assert(1 <= i && i <= _x_range_unigram);
		...
		int CRF::_hash_bigram_u(int y_i, int i, int x_i_1, int x_i){
		...
		int CRF::_hash_bigram_b(int y_i_1, int y_i, int i, int x_i_1, int x_i){
			assert(x_i_1 < _num_character_ids);
			assert(x_i < _num_character_ids);
			assert(1 <= i && i <= _x_range_bigram);
		...
		int CRF::_hash_bigram_type_u(int y_i, int type_i_1, int type_i){
		...
		int CRF::_hash_bigram_type_b(int y_i_1, int y_i, int type_i_1, int type_i){
			assert(type_i_1 < _num_character_types);
			assert(type_i < _num_character_types);
		...
		FeatureIndices* CRF::extract_features(Sentence* sentence){
			...
			for(int i = 2;i <= character_ids_length + 2;i++){	// 末尾に<eos>が2つ入る
				...
				for(int y_i = 0;y_i <= 1;y_i++){
					...
					int r_start, r_end;
					// ラベル素性
					indices_u.push_back(_hash_label_u(y_i));
					// 文字unigram素性
					...
					for(int r = r_start;r <= r_end;r++){
						int pos = r - i - _x_unigram_start + 1;	// [1, _x_range_unigram]
						int x_i = (r <= character_ids_length) ? character_ids[r - 1] : SPECIAL_CHARACTER_END;
						indices_u.push_back(_hash_unigram_u(y_i, pos, x_i));
					}
					// 文字bigram素性
					...
					for(int r = r_start;r <= r_end;r++){
						...
						int x_i = (r <= character_ids_length) ? character_ids[r - 1] : SPECIAL_CHARACTER_END;
						int x_i_1 = (r - 1 <= character_ids_length) ? character_ids[r - 2] : SPECIAL_CHARACTER_END;
						indices_u.push_back(_hash_bigram_u(y_i, pos, x_i_1, x_i));
					}
					// identical_1素性
					...
					for(int r = r_start;r <= r_end;r++){
						...
						int x_i_1 = (r - 1 <= character_ids_length) ? character_ids[r - 2] : SPECIAL_CHARACTER_END;
						if(x_i == x_i_1){
							indices_u.push_back(_hash_identical_1_u(y_i, pos));
						}
					}
					// identical_2素性
					...
					for(int r = r_start;r <= r_end;r++){
						...
						int x_i_2 = (r - 2 <= character_ids_length) ? character_ids[r - 3] : SPECIAL_CHARACTER_END;
						if(x_i == x_i_2){
							indices_u.push_back(_hash_identical_2_u(y_i, pos));
						}
					}
					// 文字種unigram・bigram素性
					int type_i = (i <= character_ids_length) ? ctype::get_type(characters[i - 1]) : CTYPE_UNKNOWN;
					int type_i_1 = (i - 1 <= character_ids_length) ? ctype::get_type(characters[i - 2]) : CTYPE_UNKNOWN;
					indices_u.push_back(_hash_unigram_type_u(y_i, type_i));
					indices_u.push_back(_hash_bigram_type_u(y_i, type_i_1, type_i));
			...
			for(int i = 2;i <= character_ids_length + 2;i++){	// 末尾に<eos>が2つ入る
				...
				for(int y_i_1 = 0;y_i_1 <= 1;y_i_1++){
					...
					for(int y_i = 0;y_i <= 1;y_i++){
						...
						int r_start, r_end;
						// ラベル素性
						indices_b.push_back(_hash_label_b(y_i_1, y_i));
						// 文字unigram素性
						r_start = std::max(1, i + _x_unigram_start);
						r_end = std::min(character_ids_length + 2, i + _x_unigram_end);	// <eos>2つを考慮
						for(int r = r_start;r <= r_end;r++){
							int pos = r - i - _x_unigram_start + 1;	// [1, _x_range_unigram]
							int x_i = (r <= character_ids_length) ? character_ids[r - 1] : SPECIAL_CHARACTER_END;
							indices_b.push_back(_hash_unigram_b(y_i_1, y_i, pos, x_i));
						}
						// 文字bigram素性
						r_start = std::max(2, i + _x_bigram_start);
						r_end = std::min(character_ids_length + 2, i + _x_bigram_end);
						for(int r = r_start;r <= r_end;r++){
							...
							int x_i = (r <= character_ids_length) ? character_ids[r - 1] : SPECIAL_CHARACTER_END;
							int x_i_1 = (r - 1 <= character_ids_length) ? character_ids[r - 2] : SPECIAL_CHARACTER_END;
							indices_b.push_back(_hash_bigram_b(y_i_1, y_i, pos, x_i_1, x_i));
						}
						// identical_1素性
						r_start = std::max(2, i + _x_identical_1_start);
						r_end = std::min(character_ids_length + 2, i + _x_identical_1_end);
						for(int r = r_start;r <= r_end;r++){
							...
							int x_i_1 = (r - 1 <= character_ids_length) ? character_ids[r - 2] : SPECIAL_CHARACTER_END;
							if(x_i == x_i_1){
								indices_b.push_back(_hash_identical_1_b(y_i_1, y_i, pos));
							}
						}
						// identical_2素性
						r_start = std::max(3, i + _x_identical_2_start);
						...
						for(int r = r_start;r <= r_end;r++){
							...
							int x_i_2 = (r - 2 <= character_ids_length) ? character_ids[r - 3] : SPECIAL_CHARACTER_END;
							if(x_i == x_i_2){
								indices_b.push_back(_hash_identical_2_b(y_i_1, y_i, pos));
							}
						}
						// 文字種unigram・bigram素性
						int type_i = (i <= character_ids_length) ? ctype::get_type(characters[i - 1]) : CTYPE_UNKNOWN;
						int type_i_1 = (i - 1 <= character_ids_length) ? ctype::get_type(characters[i - 2]) : CTYPE_UNKNOWN;
						indices_b.push_back(_hash_unigram_type_b(y_i_1, y_i, type_i));
						indices_b.push_back(_hash_bigram_type_b(y_i_1, y_i, type_i_1, type_i));
